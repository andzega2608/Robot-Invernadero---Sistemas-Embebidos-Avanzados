...
const char* topicAuto = "Sensor/Estado";
const char* topicVelocidad = "Sensor/Velocidad";
...
void applyMotorCooler() {
  ...
  if (modoAutomatico)
  {
    ...
    ledcWrite(canalA, velocidadManual);
    nuevoEstado = "MANUAL";
  }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  else if (String(topic) == topicVelocidad)
  {
    velocidadManual = message.toInt();
    velocidadManual = constrain(velocidadManual, 0, 255);
  }
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP32Client1")) {
      ...
      client.subscribe(topicVelocidad);
